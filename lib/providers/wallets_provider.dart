...
class WalletsNotifier extends Notifier<List<WalletEntry>> {
  ...
  List<WalletEntry> _loadOrMigrate() {
    ...
    if (json != null && json.isNotEmpty) {
      try {
        ...
        debugPrint('[Wallets] failed to parse wallets JSON: $e');
        return [];
      }
    }
    ...
    Future.microtask(() async {
      ...
      debugPrint('[Wallets] migrated single wallet → id=$id');
    });
  ...
  Future<void> scan(String id) async {
    ...
    try {
      ...
      debugPrint('[Wallets] scan failed ($id): $e');
      _updateEntry(
        id,
        (w) => w.copyWith(
          isScanning: false,
      ...
      debugPrint('[Wallets] scan error ($id): $e');
      _updateEntry(
        id,
        (w) => w.copyWith(
          isScanning: false,
