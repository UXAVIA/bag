...
class PriceNotifier extends AsyncNotifier<PriceData> {
  ...
  Future<PriceData> _fetchAndSideEffects(
    ...
    try {
      ...
      // Also runs on the throwing path: having no cached price to fall back
      // on is exactly when the user most needs to know Orbot is the reason.
      _syncTorStatus();
    }
  ...
  /// Mirrors the price layer's Tor gate result onto [torStatusProvider], so
  /// the home screen can explain a frozen price instead of just showing an
  /// unexplained stale timestamp.
  ///
  /// [PriceService] probes on every gated request anyway, so this reuses that
  /// result rather than paying for another probe — on iOS a probe is a round
  /// trip to check.torproject.org, which is far too expensive to repeat just
  /// to render a label.
  void _syncTorStatus() {
    if (_disposed) return;
    if (!ref.read(networkSettingsProvider).useTor) return;
    ref.read(torStatusProvider.notifier).setValue(
          ref.read(priceServiceProvider).lastRequestBlockedByTor
