...
/// Derives the address type from a Bitcoin mainnet address.
///
/// Witness v0 uses the same `bc1q` prefix for both a 20-byte key hash
/// (P2WPKH, 42 chars) and a 32-byte script hash (P2WSH — multisig, 62 chars),
...
/// Groups address types into the families a chain-analysis heuristic actually
/// keys on: legacy vs nested segwit vs native segwit vs taproot.
///
/// P2WPKH and P2WSH share a family deliberately. Holding a single-sig wallet
/// alongside a multisig is ordinary portfolio hygiene, not a fingerprint —
/// penalising it would mean every Bitkey user lost points for owning a second
/// wallet. Mixing *generations* (a legacy `1…` UTXO next to native segwit) is
/// the signal worth flagging.
String? _addressFamily(String? type) => switch (type) {
      'p2wpkh' || 'p2wsh' => 'segwit-v0',
      'p2tr' => 'taproot',
      'p2sh' => 'p2sh',
      'p2pkh' => 'legacy',
      _ => null,
...
ChainAnalysisResult analyse({
  ...
  // Determine the dominant address family to detect mixed-type UTXOs.
  final familyCounts = <String, int>{};
  for (final pair in allPairs) {
    final f = _addressFamily(_addressType(pair.address));
    if (f != null) familyCounts[f] = (familyCounts[f] ?? 0) + 1;
  }
  final dominantFamily = familyCounts.isEmpty
      ? null
      : familyCounts.entries.reduce((a, b) => a.value >= b.value ? a : b).key;
  ...
  for (final pair in allPairs) {
    ...
    final addrFamily = _addressFamily(addrType);
    if (addrFamily != null &&
        dominantFamily != null &&
        addrFamily != dominantFamily) {
