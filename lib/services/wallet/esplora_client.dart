...
class EsploraClient {
  ...
  Future<List<RawUtxo>> fetchUtxos(String address) async {
    try {
      ...
      debugPrint('[Esplora] fetchUtxos GET (address redacted)');
      final response = await _withRetry(
      ...
      if (e.message.contains('HTTP 404')) {
        ...
        if (root != baseUrl) {
          ...
          debugPrint('[Esplora] fetchUtxos 404 on primary, trying root fallback');
          try {
            ...
            debugPrint('[Esplora] fetchUtxos root fallback failed: $fe');
          }
        }
        return [];
      }
