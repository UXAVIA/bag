...
const _androidWidgetName = 'app.bitbag.BagWidget';
const _taskId = 'bag_widget_refresh';
const _widgetChannel = MethodChannel('app.bitbag/widget');

// Duplicated here so the background isolate is self-contained (no Riverpod).
...
// Currencies served by bitbag.app /api/chart (mirrors PriceService._bitbagChartCurrencies).
const _bitbagChartCurrencies = {'usd', 'eur', 'gbp', 'cad', 'jpy', 'chf', 'aud', 'xau'};
...
void backgroundWidgetCallback() {
  Workmanager().executeTask((taskName, inputData) async {
    ...
    // The portfolio amount is encrypted at rest — read it through the store,
    // never straight from SharedPreferences.
    ...
    final torOk = !useTor || await TorService.probe() == TorStatus.available;
    if (useTor && !torOk) {
    ...
    // ── Fetch current price ───────────────────────────────────────────────────
    // Primary: bitbag.app (multi-source averaged, no rate limits, all 28 currencies).
    // Fallback: CoinGecko.
    ...
    if (!useTor || torOk) {
      try {
        final response = await dio.get<Map<String, dynamic>>(
          '${AppConstants.bitbagApiBase}/price',
        );
        final rawPrices  = response.data!['prices']    as Map<String, dynamic>;
        final rawChanges = response.data!['changes24h'] as Map<String, dynamic>? ?? {};
        price     = rawPrices.containsKey(currency)  ? (rawPrices[currency]  as num).toDouble() : null;
        change24h = rawChanges.containsKey(currency) ? (rawChanges[currency] as num).toDouble() : null;
        if (kDebugMode) debugPrint('[WidgetBg] bitbag.app price: $currency = $price');
      } catch (e) {
        if (kDebugMode) debugPrint('[WidgetBg] bitbag.app price failed, trying CoinGecko: $e');
        try {
          final response = await dio.get<Map<String, dynamic>>(
          ...
          if (kDebugMode) debugPrint('[WidgetBg] price fetch failed: $e2');
        }
      }
    }

    // ── Fetch chart data ──────────────────────────────────────────────────────
    // Mirrors PriceService._doFetch routing:
    //   Primary (days 1/7/30 + supported currency): bitbag.app /api/chart
    //   Fallback / long timeframes (1Y/5Y/ALL):     Kraken OHLC

    List<double>? chartPrices;
    double? tfChange; // timeframe % change (replaces 24h change if available)
    if (!useTor || torOk) {
      bool chartFetched = false;

      // ── bitbag.app chart (primary for short timeframes) ──────────────────
      if ([1, 7, 30].contains(timeframeDays) &&
          _bitbagChartCurrencies.contains(currency)) {
        try {
          final response = await dio.get<Map<String, dynamic>>(
            '${AppConstants.bitbagApiBase}/chart',
            queryParameters: {'currency': currency, 'days': timeframeDays},
          );
          final d = response.data!['d'] as List<dynamic>;
          final fetched = d
              .map((row) => (row as List<dynamic>)[1] as num)
              .map((n) => n.toDouble())
              .toList();
          if (fetched.length >= 2) {
            chartPrices = fetched;
            final first = fetched.first;
            if (first > 0) tfChange = (fetched.last - first) / first * 100;
            chartFetched = true;
            if (kDebugMode) debugPrint('[WidgetBg] bitbag.app chart: ${fetched.length} pts for $currency (${timeframeDays}d)');
          }
        } catch (e) {
          if (kDebugMode) debugPrint('[WidgetBg] bitbag.app chart failed, trying Kraken: $e');
        }
      }

      // ── Kraken OHLC (long timeframes + fallback) ─────────────────────────
      if (!chartFetched) {
        try {
          final List<double> fetched;
          if (currency == krakenGoldCurrency) {
            // BTC in gold = BTC/USD ÷ PAXG/USD, candle by candle.
            final btcUsd = await _widgetKrakenCloses(
                dio, _widgetKrakenPairs['usd']!, timeframeDays);
            await Future.delayed(const Duration(milliseconds: 1100));
            final paxgUsd =
                await _widgetKrakenCloses(dio, krakenGoldPair, timeframeDays);
            fetched = divideAlignedTails(btcUsd, paxgUsd);
          } else {
            final pair = _widgetKrakenPairs[currency];
            fetched = pair == null
                ? const []
                : await _widgetKrakenCloses(dio, pair, timeframeDays);
          }
          if (fetched.length >= 2) {
            chartPrices = fetched;
            final first = fetched.first;
    ...
    if (wallets.isNotEmpty) {
      ...
      // A custom URL lives in secure storage, so this has to be awaited.
      final baseUrl = await resolveExplorerBaseUrl();
      ...
      if (!skipWalletScan) {
        ...
        for (final wallet in wallets) {
          // Manual entries have no addresses, and fresh wallets need no scan.
          if (!wallet.isWatchOnly || !wallet.isStale) {
            updatedWallets.add(wallet);
            continue;
          }

          try {
            final secret = await bio.readWalletSecretForScanningById(wallet.id);
            if (secret == null) {
              updatedWallets.add(wallet);
              continue;
            }
            final source = buildAddressSource(wallet.kind, secret);
            final balance = await scanWallet(source, client);
            ...
            // Never log the label or the balance — the widget isolate's log
            // output is readable by anyone with adb access on a dev device.
            if (kDebugMode) debugPrint('[WidgetBg] wallet scanned');
          } on EsploraException catch (e) {
            if (kDebugMode) debugPrint('[WidgetBg] scan failed: $e');
            updatedWallets.add(wallet);
          } catch (e) {
            if (kDebugMode) debugPrint('[WidgetBg] scan error: $e');
            updatedWallets.add(wallet);
          }
        }
    ...
    final currencyFmt = currencyFormatter(currency);
...
/// Close prices for [pair] over the widget timeframe, oldest first.
Future<List<double>> _widgetKrakenCloses(
  Dio dio,
  String pair,
  int timeframeDays,
) async {
  final response = await dio.get<Map<String, dynamic>>(
    ...
    queryParameters: _krakenParamsForWidget(pair, timeframeDays),
  );
  ...
  return ohlc.map((row) {
    ...
    return double.parse(r[4] as String);
  }).toList();
}

/// Point-wise quotient of two close series that share their most recent
/// candle. Kraken aligns every pair to the same interval boundaries and the
/// request `since` is identical, so the tails line up; only the *starts*
/// differ when one pair's history is shorter (PAXG began in 2019), which is
/// why alignment is from the end. Sparklines carry no timestamps to join on.
List<double> divideAlignedTails(List<double> numerator, List<double> denominator) {
  final n = numerator.length < denominator.length
      ? numerator.length
      : denominator.length;
  final a = numerator.sublist(numerator.length - n);
  final b = denominator.sublist(denominator.length - n);
  return [
    for (var i = 0; i < n; i++)
      if (b[i] > 0) a[i] / b[i],
  ];
...
class WidgetService {
  ...
  static Future<void> initialise() async {
    await HomeWidget.setAppGroupId(
        Platform.isIOS ? 'group.app.bitbag.shared' : 'app.bitbag');
