...
class PriceService {
  ...
  Future<PriceData> fetchCurrentPrice(List<String> currencies) async {
    try {
      ...
      debugPrint('[PriceService] price: ${response.statusCode}');
      ...
      debugPrint('[PriceService] fetchCurrentPrice error: $e\n$st');
    ...
    for (final currency in currencies) {
      for (final days in timeframes) {
        ...
        try {
          ...
          debugPrint('[PriceService] seeded $cacheKey from asset');
        } catch (_) {
  ...
  Future<double?> _historicalFromKraken(
    ...
    try {
      ...
      debugPrint('[PriceService] Kraken historical $currency ${date.toIso8601String().substring(0, 10)}: $price');
      return price;
    } catch (e) {
      debugPrint('[PriceService] Kraken historical error ($currency ${date.toIso8601String().substring(0, 10)}): $e');
      return null;
    }
  }
  ...
  Future<double?> _historicalFromCoinGecko(
    ...
    try {
      ...
      debugPrint('[PriceService] CoinGecko historical $currency $dateStr: ${response.statusCode}');
      ...
      debugPrint('[PriceService] CoinGecko historical error ($dateStr $currency): $e');
      return null;
    }
  }
  ...
  Future<void> prewarmCharts(String currency) async {
    ...
    debugPrint('[PriceService] chart pre-warm queued for $currency');
  }
  ...
  void clearChartCache(String currency, int days) {
    ...
    debugPrint('[PriceService] cleared chart cache: $key');
  }
  ...
  Future<List<(DateTime, double)>> _doFetch(
    ...
    try {
      ...
      if (days == AppConstants.timeframeAllDays ||
        ...
        debugPrint('[PriceService] CoinGecko chart ${days}d: ${response.statusCode}');
      ...
      debugPrint('[PriceService] fetchChartData error (${days}d): $e\n$st');
      final cached = box.get(cacheKey);
      if (cached != null) {
  ...
  Future<List<(DateTime, double)>> _fetchKraken(
    ...
    if (pair == null) {
      debugPrint('[PriceService] Kraken: $currency not supported, using CoinGecko 365d');
      final response = await _dio.get<Map<String, dynamic>>(
        '/coins/bitcoin/market_chart',
        queryParameters: {
          'vs_currency': currency,
    ...
    debugPrint('[PriceService] Kraken ${days == 0 ? 'ALL' : '5Y'}: ${ohlc.length} weekly candles for $pair');
