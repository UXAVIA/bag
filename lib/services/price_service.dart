...
/// BTC priced in gold is derived, not quoted: BTC/USD ÷ PAXG/USD, both from
/// Kraken. PAXG is a 1:1 gold-backed token that tracks spot within ~0.3%, and
/// Kraken has candles for it back to October 2019 — so the ALL chart in gold
/// starts there rather than 2013.
const krakenGoldCurrency = 'xau';
const krakenGoldPair = 'PAXGUSD';

/// Point-wise quotient of two `[timestamp, close]` series joined on timestamp.
/// Candles that exist in only one series are dropped, which is what trims
/// the BTC/USD history to PAXG's shorter one.
List<(DateTime, double)> divideSeries(
  List<(DateTime, double)> numerator,
  List<(DateTime, double)> denominator,
) {
  final divisors = {for (final (t, v) in denominator) t: v};
  return [
    for (final (t, v) in numerator)
      if ((divisors[t] ?? 0) > 0) (t, v / divisors[t]!),
  ];
}
...
/// One source's answer: spot prices and, where the source has them, 24 h
/// changes, both keyed by lower-case currency code.
typedef SpotQuotes = ({Map<String, double> prices, Map<String, double> changes});

/// Fills every currency [fresh] lacks from [cached], flagging each carried
/// value in [PriceData.staleCurrencies]. Carries the price only — a day-old
/// "24 h change" refers to a window that has since moved, so it is dropped
/// rather than shown as current.
///
/// Currencies present in the cache but not requested this time ride along
/// too (as stale), so re-adding a currency shows its last figure instantly.
PriceData mergeWithCache({required PriceData fresh, PriceData? cached}) {
  if (cached == null) return fresh;
  final prices = Map<String, double>.from(fresh.prices);
  final stale = <String>{};
  for (final entry in cached.prices.entries) {
...
class PriceService {
  ...
  PriceService({this.useTor = false})
      : _dio = _buildDio(useTor, AppConstants.coinGeckoBase),
        ...
        _coinbaseDio = _buildDio(useTor, 'https://api.coinbase.com/v2');
  ...
  /// Fetches the current BTC price for [currencies].
  ///
  /// Sources are tried in order and each one is asked only for the currencies
  /// still missing, so a partial answer from the primary (bitbag.app lacks
  /// gold, the app lists ILS/RUB the backend doesn't) is topped up rather
  /// than returned as-is:
  ///   0. bitbag.app — own cache (multi-source averaged, never rate-limited)
  ///   1. CoinGecko  — full coverage incl. XAU, 24 h change included
  ///   2. Coinbase   — every fiat in one keyless call, no 24 h change
  ///   3. mempool.space — USD/EUR/GBP/CAD/JPY/CHF/AUD, no 24 h change
  ///   4. Bitfinex   — USD/EUR/GBP/JPY, 24 h change included
  ///
  /// Whatever is still missing after that is carried over from the Hive cache
  /// and flagged in [PriceData.staleCurrencies]. Previously a currency no
  /// source quoted simply vanished from the home screen — the backend served
  /// 7 of 28 currencies for months in 2026 and users saw "CHF disappears when
  /// combined with USD" (GitHub issue #2).
  Future<PriceData> fetchCurrentPrice(List<String> currencies) async {
    ...
    // rather than reaching any of the upstreams over clearnet.
    try {
      await _requireTor();
    ...
    final wanted = {for (final c in currencies) c.toLowerCase()};
    final prices = <String, double>{};
    final changes = <String, double>{};

    final sources = <(String, Future<SpotQuotes?> Function(Set<String>))>[
      ('bitbag.app', _fetchFromBitbagApi),
      ('CoinGecko', _fetchFromCoinGecko),
      ('Coinbase', _fetchFromCoinbase),
      ('mempool.space', _fetchFromMempool),
      ('Bitfinex', _fetchFromBitfinex),
    ];

    for (final (name, fetch) in sources) {
      final missing = wanted.difference(prices.keys.toSet());
      if (missing.isEmpty) break;
      try {
        final quotes = await fetch(missing);
        if (quotes == null) continue;
        for (final c in missing) {
          final price = quotes.prices[c];
          if (price == null) continue;
          prices[c] = price;
          final change = quotes.changes[c];
          if (change != null) changes[c] = change;
        }
      } catch (e) {
        if (kDebugMode) debugPrint('[PriceService] $name failed: $e');
      }
    }

    final cached = _cachedPrice();
    if (prices.isEmpty) {
      // Nothing fresh at all — the stale cache beats an error screen.
      if (cached != null) {
    ...
    if (kDebugMode && merged.staleCurrencies.isNotEmpty) {
      debugPrint('[PriceService] carried over from cache: ${merged.staleCurrencies}');
    }
    _cachePriceData(merged);
    return merged;
  }

  Future<SpotQuotes?> _fetchFromCoinGecko(Set<String> currencies) async {
    ...
    for (final currency in currencies) {
      final price = btcData[currency] as num?;
      if (price == null) continue;
      prices[currency] = price.toDouble();
      final change = btcData['${currency}_24h_change'] as num?;
      if (change != null) changes[currency] = change.toDouble();
    }
    return (prices: prices, changes: changes);
  }

  Future<SpotQuotes?> _fetchFromBitbagApi(Set<String> currencies) async {
    ...
    for (final currency in currencies) {
      final price = rawPrices[currency] as num?;
      if (price == null) continue;
      prices[currency] = price.toDouble();
      final change = rawChanges[currency] as num?;
      if (change != null) changes[currency] = change.toDouble();
    }
    return (prices: prices, changes: changes);
  }

  /// Coinbase's keyless `exchange-rates` endpoint quotes BTC in every fiat
  /// the app lists in a single call (10 000 req/h, no geo-blocking), which
  /// makes it the broadest fallback when both caches above are unreachable.
  /// It carries no 24 h change and no XAU.
  Future<SpotQuotes?> _fetchFromCoinbase(Set<String> currencies) async {
    final response = await _coinbaseDio.get<Map<String, dynamic>>(
      '/exchange-rates',
      queryParameters: {'currency': 'BTC'},
    );
    if (kDebugMode) debugPrint('[PriceService] Coinbase rates: ${response.statusCode}');
    ...
    for (final currency in currencies) {
      ...
      if (price != null && price > 0) prices[currency] = price;
    }
    if (prices.isEmpty) return null;
    return (prices: prices, changes: const <String, double>{});
  }
  ...
  Future<SpotQuotes?> _fetchFromMempool(Set<String> currencies) async {
    if (!currencies.any(_mempoolSupportedCurrencies.contains)) return null;
    ...
    for (final currency in currencies) {
      if (!_mempoolSupportedCurrencies.contains(currency)) continue;
      final price = data[currency.toUpperCase()] as num?;
      if (price != null) prices[currency] = price.toDouble();
    }
  ...
  Future<SpotQuotes?> _fetchFromBitfinex(Set<String> currencies) async {
    final supported =
        currencies.where(_bitfinexSupportedCurrencies.contains).toList();
  ...
  Future<double?> fetchHistoricalPrice(String currency, DateTime date) async {
    ...
    if (lower == krakenGoldCurrency || _krakenPairs.containsKey(lower)) {
  ...
  /// Fetches the daily closing price from Kraken for the given date, caching
  /// it permanently under [cacheKey]. Gold is BTC/USD ÷ PAXG/USD on that day.
  Future<double?> _historicalFromKraken(
    ...
    try {
      final double? price;
      if (currency == krakenGoldCurrency) {
        final btcUsd = await _krakenDailyClose(_krakenPairs['usd']!, date);
        await Future.delayed(_krakenCourtesyDelay);
        final paxgUsd = await _krakenDailyClose(krakenGoldPair, date);
        price = btcUsd != null && paxgUsd != null && paxgUsd > 0
            ? btcUsd / paxgUsd
            : null;
      } else {
        price = await _krakenDailyClose(_krakenPairs[currency]!, date);
      }
  ...
  /// Kraken's public endpoints tolerate roughly one request per second.
  static const _krakenCourtesyDelay = Duration(milliseconds: 1100);

  /// Requests daily candles (interval=1440) anchored just before [date] so the
  /// target candle is always included, and picks the one whose open time is
  /// closest to the start of [date] in UTC. Returns null when the closest
  /// candle is more than two days off — the pair simply has no data that far
  /// back, and a wildly wrong price is worse than a fallback.
  Future<double?> _krakenDailyClose(String pair, DateTime date) async {
    final dayStartSec =
        DateTime.utc(date.year, date.month, date.day).millisecondsSinceEpoch ~/
            1000;

    final ohlc = await _krakenOhlc(pair, {
      'pair': pair,
      'interval': 1440,
    ...
    return double.parse(best[4] as String); // index 4 = close
  }

  /// One Kraken OHLC call, unwrapped to the candle list. Kraken returns the
  /// canonical pair name as the result key, which does not always equal the
  /// requested name, so fall back to the first non-`last` key.
  Future<List<dynamic>> _krakenOhlc(
    String pair,
  ...
  static const _bitbagChartCurrencies = {
    'usd', 'eur', 'gbp', 'cad', 'jpy', 'chf', 'aud', 'xau',
  };
  ...
  /// Gold is derived from the BTC/USD and PAXG/USD series.
  ...
  Future<List<(DateTime, double)>> _fetchKraken(
    ...
    final lower = currency.toLowerCase();
    if (lower == krakenGoldCurrency) {
      final btcUsd = await _krakenWeeklyCloses(_krakenPairs['usd']!, days);
      await Future.delayed(_krakenCourtesyDelay);
      final paxgUsd = await _krakenWeeklyCloses(krakenGoldPair, days);
      return divideSeries(btcUsd, paxgUsd);
    }

    final pair = _krakenPairs[lower];
    if (pair == null) {
    ...
    return _krakenWeeklyCloses(pair, days);
  }

  Future<List<(DateTime, double)>> _krakenWeeklyCloses(
    String pair,
    int days,
  ) async {
    ...
    final ohlc = await _krakenOhlc(pair, queryParams);
