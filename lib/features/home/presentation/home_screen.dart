...
// Timeframes served by bitbag.app's bulk chart endpoint — refreshed together
// by pull-to-refresh in a single request.
const _shortTimeframeDays = [1, 7, 30];

// Remaining timeframes, sourced from Kraken. Refreshed on their own staleness
// schedule (24 h for 1Y, 7 d for 5Y/ALL) rather than on every swipe — they
// barely move, and forcing a fetch would cost a Kraken round trip per swipe.
const _longTimeframeDays = [
  365,
  AppConstants.timeframe5YDays,
  AppConstants.timeframeAllDays,
];

/// One-shot per process, like `_splashShown` in app.dart: the rating prompt
/// bookkeeping runs once per cold start, not on every rebuild.
...
class HomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (!_reviewPromptScheduled) {
      ...
      final hasHoldings = portfolio.hasAmount;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ReviewPromptService.onHomeShown(hasHoldings: hasHoldings);
      });
    }
      ...
      body: RefreshIndicator(
        ...
        onRefresh: () async {
          final currency = portfolio.selectedCurrencies.isNotEmpty
              ? portfolio.selectedCurrencies.first
              : 'usd';
          // Charts, spot price and wallet balances refresh concurrently so the
          // spinner reflects the slowest one rather than their sum.
          //
          // fetchAllCharts pulls 1D/7D/30D in a single request, so switching
          // timeframes after the swipe needs no further fetch. Previously only
          // the visible timeframe was refreshed (to avoid CoinGecko 429s),
          // which left the others showing stale data until they aged out.
          await Future.wait([
            ref.read(priceServiceProvider).fetchAllCharts(currency),
            ref.read(priceProvider.notifier).refresh(force: true),
            ref.read(walletsProvider.notifier).scanAllIfStale(),
          ]);

          // Re-read every timeframe now the cache is updated. For the long ones
          // a fresh cache makes this a no-op; a stale one kicks off a
          // background refresh that now repaints when it lands rather than
          // being silently dropped.
          for (final days in [..._shortTimeframeDays, ..._longTimeframeDays]) {
            ref.invalidate(chartDataProvider((currency, days)));
          }
        },
...
class _BtcAmountRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Tapping the balance jumps to wherever that balance is actually managed:
    // the wallet list for Pro users, the BTC amount field for everyone else.
    // The unit icon keeps its own tap target for the BTC/sats toggle.
    final proUnlocked = ref.watch(proUnlockedProvider);
    final destination = proUnlocked ? '/settings/wallet-privacy' : '/settings';
                ...
                children: [
                  ...
                  if (hasWallets) ...[
                    ...
                    Tooltip(
                      message: isScanning
                          ? 'Scanning wallet…'
                          : 'Balance from your connected holdings',
                      child: isScanning
                          ? const SizedBox(
                              width: 12,
                              height: 12,
...
/// Re-probes Orbot and, if it is back, pulls prices immediately rather than
/// leaving the user to wait out the 5-minute refresh timer — they tapped
/// because they just started Orbot.
///
/// Callers must check `mounted` after awaiting.
Future<void> _recheckOrbotAndRefresh(WidgetRef ref) async {
  await ref.read(torStatusProvider.notifier).recheck();
  if (ref.read(torStatusProvider).valueOrNull == TorStatus.available) {
    await ref.read(priceProvider.notifier).refresh(force: true);
  }
}
...
class _ErrorCardState extends ConsumerState<_ErrorCard> {
  ...
  Future<void> _recheck() async {
    ...
    try {
      await _recheckOrbotAndRefresh(ref);
    } finally {
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    // Reached only when there was no cached price to fall back on, so when the
    // Tor gate threw it is the entire reason this screen is empty. Saying
    // "unable to fetch price" here would point the user at their connection
    // when the app deliberately declined to make the request.
    final torBlocked = widget.error is TorUnavailableException;
    ...
    return Container(
      ...
      child: Column(
        children: [
          Icon(
            torBlocked ? Icons.router_outlined : Icons.wifi_off_outlined,
            color: torBlocked
                ? AppColors.negative
          ...
          const SizedBox(height: 12),
          Text(
            torBlocked ? 'Orbot not detected' : 'Unable to fetch price',
            style: Theme.of(context).textTheme.bodyLarge,
          ),
          const SizedBox(height: 4),
          Text(
            torBlocked
                ? 'Tor is on, so prices are never fetched over clearnet. '
                    'Start Orbot, then re-check.'
                : 'Pull down to retry',
            textAlign: TextAlign.center,
            style: Theme.of(context).textTheme.bodySmall,
          ),
          ...
          if (torBlocked)
            TextButton(
              ...
              child: Text(_rechecking ? 'Checking…' : 'Re-check Orbot'),
            )
          else
            TextButton(
...
class _LastUpdatedRow extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_LastUpdatedRow> createState() => _LastUpdatedRowState();
}

class _LastUpdatedRowState extends ConsumerState<_LastUpdatedRow> {
  bool _rechecking = false;
  ...
  @override
  Widget build(BuildContext context) {
    final formatted = DateFormat('HH:mm').format(widget.fetchedAt);
    final smallText = Theme.of(context).textTheme.bodySmall;

    // Tor dropping freezes every price path at once, and the cascade answers
    // with cached data rather than an error — by design, so no clearnet
    // request escapes. The cost is that a frozen price is indistinguishable
    // from a current one, so name the cause here instead of showing a bare
    // "stale". Only watch the probe when Tor is on: && short-circuits, so the
    // dependency (and its network probe) is never registered otherwise.
    final torDown =
        ref.watch(networkSettingsProvider.select((s) => s.useTor)) &&
            ref.watch(torStatusProvider).valueOrNull == TorStatus.unavailable;

    if (torDown) {
      return Row(
        children: [
          Icon(Icons.router_outlined, size: 12, color: AppColors.negative),
          const SizedBox(width: 4),
          Flexible(
            child: Text(
              'Orbot not detected — prices paused since $formatted',
              style: smallText?.copyWith(color: AppColors.negative),
              overflow: TextOverflow.ellipsis,
            ),
          ),
          const SizedBox(width: 6),
    ...
    return Row(
      children: [
        Icon(
          ...
          size: 12,
          color: widget.isStale ? AppColors.negative : smallText!.color!,
        ),
        const SizedBox(width: 4),
        Flexible(
          child: Text(
            widget.isStale
                ? 'Stale data — last updated $formatted'
                : 'Updated $formatted',
            style: smallText,
            overflow: TextOverflow.ellipsis,
          ),
