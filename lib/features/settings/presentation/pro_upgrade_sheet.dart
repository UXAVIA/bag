...
class _ProUpgradeSheetState extends ConsumerState<_ProUpgradeSheet> {
  ...
  void initState() {
    super.initState();
    if (AppConstants.kFlavor == 'store') _loadProductDetails();
  }
  ...
  Future<void> _loadProductDetails() async {
    ...
    if (response.productDetails.isNotEmpty) {
      ...
      final notFound = response.notFoundIDs.isNotEmpty;
      setState(() => _iapError = err ??
          (notFound ? 'Product not found (${response.notFoundIDs.join(', ')})' : 'Could not load product'));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isPlayStore = AppConstants.kFlavor == 'store';
      ...
      child: SingleChildScrollView(
        ...
        children: [
          ...
          if (isPlayStore) ...[
            ...
            const SizedBox(height: 20),
            if (_storeUnavailable || _iapError != null) ...[
              Text(
                _storeUnavailable
                    ? 'App Store unavailable. Check your device settings.'
                    : _iapError!,
