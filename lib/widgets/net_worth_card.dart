...
class NetWorthCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // A currency no source quoted still gets its card. Hiding it (the old
    // behaviour) read as "my currency disappeared" — GitHub issue #2.
    final btcPrice = priceData.priceFor(currencyCode);
    final carriedOver = priceData.isCarriedOver(currencyCode);
    final change = btcPrice == null
        ? null
        : timeframeChange ?? priceData.changeFor(currencyCode);

    final netWorth =
        btcPrice == null ? null : portfolio.netWorthIn(currencyCode, btcPrice);
    final isPositive = (change ?? 0) >= 0;

    final worthFormatter = info.formatter();
    final priceFormatter = info.formatter(decimalDigits: 2);
      ...
      child: Column(
        ...
        children: [
          ...
          FittedBox(
            ...
            child: Text(
              portfolio.hasAmount && netWorth != null
